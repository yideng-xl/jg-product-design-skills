...
- **新增卡片** → 卡片左侧加 4px 绿色色带 + 标题后挂 `<span class="badge-new">v0.X.Y 新增</span>` 徽标(绿底白字)
- **修订内容**(文字改了) → 显示<span style="color:#909399; text-decoration:line-through;">旧版本(灰色删除线)</span> + 新版本,新版本旁挂 `v0.X.Y 修订` 徽标(橙底白字)
- **删除内容** → 保留旧位置占位,加 `v0.X.Y 已删除` 徽标(红底白字)
...
2. **完整版本变更记录,独立成章(踩坑)**:在 index 里用独立的 section 标题「**版本变更记录**」承载(与「PRD 摘要」等章节同级、同样式),**不要埋在"原型设计说明"卡片内部的表格里**——埋在卡片里评审找不到,真实评审中被产品经理要求单独立题。表字段:版本号 / 时间 / **涉及页面**(必填列) / 变更内容摘要;摘要里每条变更必须用 `<span class="ver-pages">页面名</span>` 这种 chip 标注涉及哪些页面,评审能直接跳过去看
...
> 完整标准见姐妹 skill 的 `../proto-check/assets/七大易用原则量化标准.md`(原型检查版 70 条,F/T/D/C/R/E/P)。**做原型时就按它做,proto-check 事后会按同一份标准验收**——生成时遵守一条,验收时就少一条整改。下面只列生成时最容易违规的:
...
- [ ] 长文本列省略号 + title 悬浮全文(C-13);纯图标按钮带 title 提示(R-10)
- [ ] 标准里标【说明】的条目(校验时机 / loading 规则 / 导出范围 / 离开拦截等)不要求做交互,**写进 proto-note 业务规则**即可
