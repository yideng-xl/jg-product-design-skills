...
4. **共性 / 全局说明的固定区(铁律)**:凡是"全产品统一、不随单个页面变"的说明,集中放在 index 一处,不在各业务页 proto-note 重复。典型是**数据权限**——菜单 / 按钮 / 数据三级权限**统一在权限管理处维护**(地址:https://365.kdocs.cn/l/cuTDkjddouuf),原型**不逐页画权限矩阵**,只在 index 固定放一条「数据权限说明」指向该维护处即可(proto-check 的 Z-整体-10 据此判定:index 有此固定说明即达标)。
