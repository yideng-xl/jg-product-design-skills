...
| 文件 | 内容 | 用途 |
|---|---|---|
| `assets/产品设计自查表.md` | 67 条,按功能类型分组(表单/导入/上传/列表/统计/报表/报告/树/配置/推送/工具/接口/整体),编号 `Z-*` | **产品自查报告**的依据 |
| `assets/七大易用原则量化标准.md` | **原型检查版**:原规范 75 条(写给生产系统)已逐条改写为原型检查口径,原型版 70 条(F/T/D/C/R/E/P)+ 5 条纯生产条目分流「上线前核验清单」 | **UI 规范自查报告**的依据;判定一律以「原型检查口径」列为准,不按规范原文判 |
...
每条标准的判定手段已标注在「原型检查口径」列的标签里:**【源码】/【渲染】/【说明】/【人工】**。纯生产条目(响应时长、防抖等 5 条)不判、不计符合度,原样附在 UI 规范自查报告末尾供提测/验收用。
...
2. `UI规范自查报告-YYYYMMDD-claude.md` —— 依据七大原则量化标准(原型检查版);**末尾原样附「上线前核验清单」5 条**,不计入符合度
