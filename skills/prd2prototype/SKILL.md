...
- index 顶部放一行「**改动定位：**」图例,展示这三个徽标 + 一句"在改动过的页面里,原型说明区会带这种徽标,让你一眼定位"。
...
- **原型中 vs 原型说明中(两种载体,同色同措辞)**:
  - ① **原型中**(改动元素旁):挂**可点开说明的彩色徽标**——徽标用 `.chg-badge`(三色+类型词),**点击弹出可复制的需求说明**。因为界面上没地方写长说明,靠弹层补。
  - ② **原型说明中**(proto-note / 字段规格表 / 说明卡):用**同款三色徽标,但静态、不带弹框**——因为说明文字本来就在徽标旁边,不需要再弹。徽标紧跟在被改的那行/那段文字旁即可。
